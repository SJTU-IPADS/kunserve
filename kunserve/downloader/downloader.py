...
FlashTransformer's internal layout.

Default path where facebook/opt-125m is saved:
    ~/.cache/huggingface/kunserve/models--facebook--opt-125m/
"""
import filelock
import os
import torch

from kunserve.config import ModelConfig
...
default_cache = os.path.join(os.path.expanduser("~"), ".cache", "huggingface", "kunserve")
KUNSERVE_CACHE = os.getenv("KUNSERVE_CACHE", default_cache)
